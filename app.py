        ...
        with conn.cursor() as cursor:
            ...
            for op in ops_to_create_rules:
                ...
                rules = [
                    ...
                    ('Call de Acompanhamento', op['call_frequency'], 'Call de acompanhamento.'),
                    ('Análise de DFs & Dívida', op['df_frequency'], 'Análise dos DFs.')
                ]
                
                if op.get('monitoring_news'):
                    rules.append(('Monitorar Notícias', 'Semanal', 'Acompanhar notícias.'))
                
                for name, freq, desc in rules:
                    cursor.execute("""
